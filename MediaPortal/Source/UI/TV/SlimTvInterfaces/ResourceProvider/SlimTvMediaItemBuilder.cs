...
namespace MediaPortal.Plugins.SlimTv.Interfaces.ResourceProvider
{
  public class SlimTvMediaItemBuilder
  {
    ...
    private static LiveTvMediaItem.LiveTvMediaItem CreateCommonMediaItem(int slotIndex, string path, bool isTv)
    {
      ...
      if (isTv)
      {
        providerResourceAspect.SetAttribute(ProviderResourceAspect.ATTR_MIME_TYPE, LiveTvMediaItem.LiveTvMediaItem.MIME_TYPE_TV); // Custom mimetype for TV
        providerResourceAspect.SetAttribute(ProviderResourceAspect.ATTR_RESOURCE_TYPE, ProviderResourceAspect.TYPE_VIDEO);

        // VideoAspect needs to be included to associate VideoPlayer later!
        MultipleMediaItemAspect videoAspect = MediaItemAspect.CreateAspect(aspects, VideoAspect.Metadata);
        videoAspect.SetAttribute(VideoAspect.ATTR_RESOURCE_INDEX, 0);
        videoAspect.SetAttribute(VideoAspect.ATTR_STREAM_INDEX, -1);

        MediaItemAspect.SetAttribute(aspects, MediaAspect.ATTR_TITLE, "Live TV");
      }
      else
      {
        providerResourceAspect.SetAttribute(ProviderResourceAspect.ATTR_MIME_TYPE, LiveTvMediaItem.LiveTvMediaItem.MIME_TYPE_RADIO); // Custom mimetype for Radio
        ...
        MediaItemAspect.SetAttribute(aspects, MediaAspect.ATTR_TITLE, "Live Radio");
      }
